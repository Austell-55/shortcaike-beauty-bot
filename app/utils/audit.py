...
def init_db():
    ...
    cur.execute('''
        CREATE TABLE IF NOT EXISTS orders (
            ...
            items TEXT,
            total REAL,
            status TEXT DEFAULT 'pending_availability',
    ...
    cur.execute("INSERT OR IGNORE INTO delivery_fees (location, fee) VALUES ('Lagos Island', 1500)")
    cur.execute("INSERT OR IGNORE INTO delivery_fees (location, fee) VALUES ('Abuja Central', 2000)")
    cur.execute("INSERT OR IGNORE INTO delivery_fees (location, fee) VALUES ('Port Harcourt', 1800)")

    # Sample products
    cur.execute("INSERT OR IGNORE INTO products (name, price, stock, available) VALUES ('Hydrating Serum', 7000, 20, 1)")
    cur.execute("INSERT OR IGNORE INTO products (name, price, stock, available) VALUES ('Brightening Face Cream', 5500, 15, 1)")
    cur.execute("INSERT OR IGNORE INTO products (name, price, stock, available) VALUES ('Acne Treatment Gel', 4500, 10, 1)")
    ...
    conn.commit()
    conn.close()
    print("✅ Database initialized (no test data).")
...
async def api_add_user(request: Request):
    ...
    password = form.get("password") or secrets.token_urlsafe(6)
    try:
...
"""
        email_sent = await send_email(email, f"Welcome to Shortcaike Dashboard – {role} access", body)
        return JSONResponse({"status": "ok", "password": password, "email_sent": email_sent})
    ...
    with get_db() as conn:
        if status and status != "all":
            if status == "pending_availability":
                ...
                return JSONResponse({"pending_avail": [dict(o) for o in orders], "awaiting_payment": [], "delivered_confirm": []})
            elif status == "awaiting_payment":
                orders = conn.execute("SELECT * FROM orders WHERE status='awaiting_payment'").fetchall()
                return JSONResponse({"pending_avail": [], "awaiting_payment": [dict(o) for o in orders], "delivered_confirm": []})
                ...
                return JSONResponse({"pending_avail": [], "awaiting_payment": [], "delivered_confirm": [dict(o) for o in orders]})
    ...
    with get_db() as conn:
        ...
        if order["customer_phone"]:
            ...
            try:
                ...
                session = get_session(order["customer_phone"])
                update_session(order["customer_phone"], pending_order_id=order_id, step="awaiting_payment_method")
            except:
                pass
    return JSONResponse({"status": "ok"})
...
async def api_confirm_payment(request: Request):
    ...
    order_id = form.get("order_id")
    with get_db() as conn:
        order = conn.execute("SELECT status, payment_status, customer_phone, customer_name, total FROM orders WHERE order_id=?", (order_id,)).fetchone()
        if not order:
            return JSONResponse({"error": "Order not found"}, 404)
        old_status = order["status"]
...
def api_monitor_conversations(request: Request):
    ...
    with get_db() as conn:
        convs = conn.execute("""
            SELECT 
                m.user_id as customer_phone,
                COALESCE(c.name, m.user_id) as customer_name,
...
def api_monitor_flagged(request: Request):
    ...
    with get_db() as conn:
        flagged = conn.execute("""
            SELECT f.id, f.customer_phone, f.reason, COALESCE(c.name, f.customer_phone) as customer_name
...
@app.post("/api/monitor/resolve")
async def api_monitor_resolve(request: Request):
    user = get_current_user(request)
    if not user or user["role"] not in ["monitor", "admin"]:
        return JSONResponse({"error": "Unauthorized"}, 403)
    form = await request.form()
    flag_id = int(form.get("flag_id"))
    with get_db() as conn:
        conn.execute("UPDATE conversation_flags SET resolved = 1 WHERE id = ?", (flag_id,))
        conn.commit()
        log_audit(user["id"], user["username"], user["role"], "RESOLVE_FLAG", str(flag_id), request.client.host)
    return JSONResponse({"status": "ok"})
...
def api_audit_logs(request: Request, action: str = None, user: str = None, from_date: str = None, to_date: str = None):
    ...
    if not user_obj or user_obj["role"] not in ["security", "admin"]:
        return JSONResponse({"error": "Unauthorized"}, 403)
